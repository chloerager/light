...
namespace light
{
   ...
   public class PinYin
   {
      public static string GetInitial(string hz)
      {
         string initials = string.Empty;
         for (int i = 0; i < hz.Length; i++)
         {
            byte[] array = System.Text.Encoding.GetEncoding(936).GetBytes(Convert.ToString(hz[i]));
            if (array[0] >= 176 && array[0] <= 215)  //一级汉字
            {
               int k = (short)(array[0] - '\0') * 256 + ((short)(array[1] - '\0'));
               if (k< 0xB0C5) initials += "a";
               else if (k< 0xB2C1) initials += "b";
               else if (k < 0xB4EE) initials += "c";
               else if (k < 0xB6EA) initials += "d";
               else if (k < 0xB7A2) initials += "e";
               ...
               else if (k < 0xC5B6) initials += "n";
               else if (k < 0xC5BE) initials += "o";
               else if (k < 0xC6DA) initials += "p";
               else if (k < 0xC8BB) initials += "q";
               else if (k < 0xC8F6) initials += "r";
               else if (k < 0xCBFA) initials += "s";
               else if (k < 0xCDDA) initials += "t";
               else if (k < 0xCEF4) initials += "w";
               else if (k < 0xD1B9) initials += "x";
               else if (k < 0xD4D1) initials += "y";
               else if (k < 0xD7FA) initials += "z";
            }
            else if (array[0] >= 215)
            {
               initials += GetInitialLevel2(hz[i]);
            }
            else
            {
               if (hz[i] >= 'a' && hz[i] <= 'z') initials += hz[i];
               else if (hz[i] >= 'A' && hz[i] <= 'Z') initials += hz[i].ToString().ToLower();
            }
         }
         return initials;
      }

      private static string GetInitialLevel2(char hz)
      {
         string l2en = "" +
       ...
       "albxyfbpnlsfhtgjwejjxxglljstgshjqlzfkcgnndszfdeqfhbsaqtgllbxmmygszldydqmjjrgbjtkgdhgkblqkbdmbylxwcxyttybkmrtjzxqjbhlmhmjjzmqasldcyxyqdlqcafywyxqhzqhz";
         string l2ch = "亍丌兀丐廿卅丕亘丞鬲孬噩丨禺丿匕乇夭爻卮氐囟胤馗毓睾鼗丶亟" +
       ...
       "鳕鳗鳘鳙鳜鳝鳟鳢靼鞅鞑鞒鞔鞯鞫鞣鞲鞴骱骰骷鹘骶骺骼髁髀髅髂髋髌髑魅魃魇魉魈魍魑飨餍餮饕饔髟髡髦髯髫髻髭髹鬈鬏鬓鬟鬣麽麾縻麂麇麈麋麒鏖麝麟黛黜黝黠黟黢黩黧黥黪黯鼢鼬鼯鼹鼷鼽鼾齄";
         ...
         int pos = l2ch.IndexOf(hz.ToString(), 0);
         if (pos < 0) return "";
         return l2en.Substring(pos, 1);
      }
   }
}
