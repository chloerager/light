(function ($) {
   ...
   function appendTreeNodes(setting, level, treeNodes, parentNode) {
      ...
      for (var i = 0, l = treeNodes.length; i < l; i++) {
         ...
         html.push("<a id='", node.tId, IDMark_A, "' treeNode", IDMark_A, " onclick=\"", (node.click || ''),
				...
				"'>", node[setting.nameCol].replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'), "(" + node['count'] + ")", "</span></a><ul id='", node.tId, IDMark_Ul, "' class='", makeUlLineClass(setting, node), "' style='display:", (node.open ? "block" : "none"), "'>");
   ...
   function setNodeName(setting, treeNode) {
      ...
      nObj.text(treeNode[setting.nameCol] + "(" + treeNode['count'] + ")");
   }
   //设置Target
   ...
   function addEditBtn(setting, treeNode) {
      if (treeNode.editNameStatus || $("#" + treeNode.tId + IDMark_Edit).length > 0 || treeNode.pid==undefined) {
         return;
      }
